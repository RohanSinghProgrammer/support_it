...
import { AlertCircle, LoaderCircle, Lock, Mail, User } from 'lucide-react'
...
export function RegisterForm() {
  ...
  const canSubmit = useMemo(
    () =>
      ...
      !passwordMismatch &&
      !isSubmitting,
    [confirmPassword, email, isSubmitting, name, password, passwordMismatch]
  )
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : 'Google sign-up is not available right now.'
      )
    ...
    <Card className="w-full max-w-md rounded-2xl border-border/70 shadow-xl">
      <CardContent className="space-y-6 p-6">
        <div className="space-y-1 text-center">
          ...
          <p className="text-sm text-muted-foreground">Sign up to get started with SupportIt</p>
        </div>
